...
function getSourceDir(fromDir, target) {
  // claude-global uses the same source as claude; cursor-global uses the same as cursor; gemini-global uses the same as gemini
  const sourceTarget =
    target === "claude-global"
      ? "claude"
      : target === "cursor-global"
      ? "cursor"
      : target === "gemini-global"
      ? "gemini"
      : target;
  const targetDirMap = {
...
function getDestDir(destRepoRoot, target) {
  // claude-global, cursor-global and gemini-global don't need destRepoRoot
